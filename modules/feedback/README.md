...
Observation-to-action recommendations and re-evaluation.
...
`live.LiveFeedbackService` 接收必填 `ObservationPort`，由组合入口注入 Analytics
的公开 `ObservationService`；本模块不直接导入 Analytics。窗口按稳定子命令键
写入四类观察，部分失败后可用原键重试。接口和恢复语义见
`docs/adr/ADR-002-live-feedback-observation-port.md`。当前仅完成本地合成数据验证，
真实来源认证、持久化接入和平台验收仍待完成。
