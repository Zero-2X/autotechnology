...
当前边界：小红书账号已在本机独立浏览器重新扫码，后台当前显示连接正常。图文草稿可排队并填入创作页，默认停在发布按钮前，由操作者在官方页面确认；网页自动点击发布默认关闭，结果仍需回查平台。客服入口不持续扫描创作者中心页面，当前采用人工导入评论/私信、生成草稿、复制到官方工具发送并登记结果。真实消息默认标为“待人工判定”，浏览器登录不等于已取得官方 API 发布授权。没有真实 S3 或平台 API 发布；PostgreSQL 专有的
...
默认打开 `http://127.0.0.1:8766/`，同时启动 API。内容工作室可以在模型服务不可用时生成本地标题、正文、话题标签和封面；客服收件箱可以生成回复草稿，复制到官方页面人工发送后登记。小红书登录会话使用独立本机浏览器目录保存，发布默认停在最终按钮前。连接与设置中的运营规则卡片提供每日/每周限额和紧急停止，发布准备会在 API 层再次检查；网页自动点击发布和自动回复底层默认关闭。跨平台策略和客服路径见 `docs/客服回复策略方案.md`、`docs/运营规则与跨平台低风险操作.md`。

“试点与排期”页面支持建立本地单账号/多账号试点，设置负责人、替补、发布上限和停发条件，并逐条登记内容计划、开始/停止/完成状态和复盘记录。试点数据只写入 `.local/pilot-runs.json`，不包含密码、Cookie、Token，也不会自动触发平台动作。
