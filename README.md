...
当前边界：小红书账号已在本机独立浏览器重新扫码，后台当前显示连接正常。图文草稿可排队填入，操作者主动确认后可以尝试提交发布，结果仍需回查平台。客服入口尚未自动识别当前创作者中心页面，本轮增加“手动进入消息页后扫描当前页”的流程；真实消息读取和回复发送仍待账号本人逐步验收。真实消息默认标为“待人工判定”，只有人工确认低风险、且平台消息 ID 稳定时才允许尝试发送。浏览器登录不等于已取得官方 API 发布授权。没有真实 S3 或平台 API 发布；PostgreSQL 专有的
...
默认打开 `http://127.0.0.1:8766/`，同时启动 API。内容工作室可以在模型服务不可用时生成本地标题、正文、话题标签和封面；客服收件箱可以生成低风险回复草稿，并在操作者主动点击后通过小红书可见页面尝试发送。后台和浏览器适配器都会拒绝中高风险回复；发送后须回查平台，再标记完成。小红书登录会话使用独立本机浏览器目录保存，发布默认停在最终按钮前，也支持主动确认后尝试提交发布。
