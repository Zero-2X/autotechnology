...
默认打开 `http://127.0.0.1:8766/`，同时启动 API。内容工作室可以在模型服务不可用时生成本地标题、正文、话题标签和封面；客服收件箱可以生成低风险回复草稿，复制到官方页面人工发送后登记。后台和浏览器适配器都会拒绝中高风险回复；小红书登录会话使用独立本机浏览器目录保存，发布默认停在最终按钮前。发布与客服策略的比较见 `docs/客服回复策略方案.md`。
