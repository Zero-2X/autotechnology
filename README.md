...
默认打开 `http://127.0.0.1:8766/`，同时启动 API。内容工作室可以在模型服务不可用时生成本地标题、正文、话题标签和封面；客服收件箱可以生成回复草稿，复制到官方页面人工发送后登记。小红书登录会话使用独立本机浏览器目录保存，发布默认停在最终按钮前。连接与设置中的运营规则卡片提供每日/每周限额和紧急停止，发布准备会在 API 层再次检查。跨平台策略和客服路径见 `docs/客服回复策略方案.md`、`docs/运营规则与跨平台低风险操作.md`。
