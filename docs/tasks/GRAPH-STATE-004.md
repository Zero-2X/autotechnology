# GRAPH-STATE-004：定义 State 迁移和版本兼容策略
...
定义 State 迁移和版本兼容策略。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-STATE-004`
...
python scripts/check_task_card_precision.py --task GRAPH-STATE-004 --strict
