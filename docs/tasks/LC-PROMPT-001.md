# LC-PROMPT-001：实现版本化 Prompt 模板和变量校验
...
实现版本化 Prompt 模板和变量校验。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `LC-PROMPT-001`
...
python scripts/check_task_card_precision.py --task LC-PROMPT-001 --strict
