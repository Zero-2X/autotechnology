# GRAPH-CORE-003：将 TaskJob 映射为 GraphRun 并保持现有状态机
...
将 TaskJob 映射为 GraphRun 并保持现有状态机。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-CORE-003`
...
python scripts/check_task_card_precision.py --task GRAPH-CORE-003 --strict
