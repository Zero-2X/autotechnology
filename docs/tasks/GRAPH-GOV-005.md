# GRAPH-GOV-005：把 Node、State、Replay 门禁写入 Codex 执行协议
...
把 Node、State、Replay 门禁写入 Codex 执行协议。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-GOV-005`
...
python scripts/check_task_card_precision.py --task GRAPH-GOV-005 --strict
