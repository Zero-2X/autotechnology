# GRAPH-DIST-001：接入 AccountConnection/TargetVersion/PolicySnapshot 新版本链
...
接入 AccountConnection/TargetVersion/PolicySnapshot 新版本链。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-REPLAY-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-DIST-001`
...
python scripts/check_task_card_precision.py --task GRAPH-DIST-001 --strict
