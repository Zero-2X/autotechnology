# GRAPH-GOV-001：冻结 LangGraph 边界并登记 ADR-001
...
冻结 LangGraph 边界并登记 ADR-001。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-GOV-001`
