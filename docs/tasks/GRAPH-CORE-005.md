# GRAPH-CORE-005：实现取消、暂停和 Kill Switch 检查
...
实现取消、暂停和 Kill Switch 检查。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-CORE-005`
...
python scripts/check_task_card_precision.py --task GRAPH-CORE-005 --strict
