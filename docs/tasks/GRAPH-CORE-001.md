# GRAPH-CORE-001：建立 GraphRegistry 和 orchestration 包
...
建立 GraphRegistry 和 orchestration 包。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-GOV-002
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-CORE-001`
...
python scripts/check_task_card_precision.py --task GRAPH-CORE-001 --strict
