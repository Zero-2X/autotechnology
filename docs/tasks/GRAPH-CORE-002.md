# GRAPH-CORE-002：实现 GraphRunner：输入校验、trace、租约、超时、结果引用
...
实现 GraphRunner：输入校验、trace、租约、超时、结果引用。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-CORE-002`
...
python scripts/check_task_card_precision.py --task GRAPH-CORE-002 --strict
