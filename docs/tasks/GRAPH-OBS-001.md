# GRAPH-OBS-001：统一 Graph/Node/Model/Outbox trace、metrics 和 cost
...
统一 Graph/Node/Model/Outbox trace、metrics 和 cost。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-OBS-001`
...
python scripts/check_task_card_precision.py --task GRAPH-OBS-001 --strict
