# LC-PARSER-001：实现结构化输出解析、校验和有限修复
...
实现结构化输出解析、校验和有限修复。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `LC-PARSER-001`
...
python scripts/check_task_card_precision.py --task LC-PARSER-001 --strict
