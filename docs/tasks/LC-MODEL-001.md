# LC-MODEL-001：实现 ModelPort 和 LangChain ChatModel Adapter
...
实现 ModelPort 和 LangChain ChatModel Adapter。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-CORE-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `LC-MODEL-001`
...
python scripts/check_task_card_precision.py --task LC-MODEL-001 --strict
