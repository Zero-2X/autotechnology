# GRAPH-ROUTER-001：实现通过、返工、人工、阻断纯函数路由
...
实现通过、返工、人工、阻断纯函数路由。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-NODE-003
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-ROUTER-001`
...
python scripts/check_task_card_precision.py --task GRAPH-ROUTER-001 --strict
