# LC-MODEL-002：实现 FakeModel 和模型故障注入
...
实现 FakeModel 和模型故障注入。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `LC-MODEL-002`
...
python scripts/check_task_card_precision.py --task LC-MODEL-002 --strict
