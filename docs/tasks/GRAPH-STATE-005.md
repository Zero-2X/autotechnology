# GRAPH-STATE-005：增加 State 敏感字段和大小 lint
...
增加 State 敏感字段和大小 lint。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-STATE-005`
...
python scripts/check_task_card_precision.py --task GRAPH-STATE-005 --strict
