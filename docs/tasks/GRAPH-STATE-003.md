# GRAPH-STATE-003：定义并行分支 Reducer 和冲突策略
...
定义并行分支 Reducer 和冲突策略。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-STATE-003`
...
python scripts/check_task_card_precision.py --task GRAPH-STATE-003 --strict
