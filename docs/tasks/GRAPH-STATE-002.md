# GRAPH-STATE-002：定义 ArtifactRef、EvidenceRef、PolicyRef、HumanInterruptRef

- **状态**：planned
- **阶段**：2 — Graph State 与契约
- **层级**：P0
- **依赖**：GRAPH-STATE-001
...
定义 ArtifactRef、EvidenceRef、PolicyRef、HumanInterruptRef。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-STATE-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-STATE-002`
...
python scripts/check_task_card_precision.py --task GRAPH-STATE-002 --strict
