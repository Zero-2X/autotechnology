# LC-TOOL-001：实现 Tool 白名单、参数 Schema、超时和审计
...
实现 Tool 白名单、参数 Schema、超时和审计。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `LC-TOOL-001`
...
python scripts/check_task_card_precision.py --task LC-TOOL-001 --strict
