# GRAPH-GOV-002：冻结 LangGraph/LangChain/Python 版本和升级窗口
...
冻结 LangGraph/LangChain/Python 版本和升级窗口。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-GOV-002`
...
python scripts/check_task_card_precision.py --task GRAPH-GOV-002 --strict
