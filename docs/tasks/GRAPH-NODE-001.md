# GRAPH-NODE-001：实现 TopicSignal 到 TopicBrief 节点

- **状态**：planned
- **阶段**：4 — 无账号内容纵向切片
- **层级**：P0
- **依赖**：GRAPH-STATE-001, LC-MODEL-001
...
实现 TopicSignal 到 TopicBrief 节点。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-STATE-001, LC-MODEL-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-NODE-001`
...
python scripts/check_task_card_precision.py --task GRAPH-NODE-001 --strict
