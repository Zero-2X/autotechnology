# GRAPH-OBS-002：把 Graph 生命周期映射到 AuditLog
...
把 Graph 生命周期映射到 AuditLog。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-OBS-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-OBS-002`
...
python scripts/check_task_card_precision.py --task GRAPH-OBS-002 --strict
