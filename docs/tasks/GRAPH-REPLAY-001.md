# GRAPH-REPLAY-001：实现 Graph 重放、PublicationIntent 和 provider 幂等保护
...
实现 Graph 重放、PublicationIntent 和 provider 幂等保护。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-NODE-004
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-REPLAY-001`
...
python scripts/check_task_card_precision.py --task GRAPH-REPLAY-001 --strict
