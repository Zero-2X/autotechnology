# GRAPH-EVAL-001：建立 Graph golden set、确定性回归和成本预算
...
建立 Graph golden set、确定性回归和成本预算。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-EVAL-001`
...
python scripts/check_task_card_precision.py --task GRAPH-EVAL-001 --strict
