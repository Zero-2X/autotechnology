# GRAPH-STATE-001：建立 BaseGraphState 与 ContentGraphState Schema
...
建立 BaseGraphState 与 ContentGraphState Schema。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-GOV-003
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-STATE-001`
...
python scripts/check_task_card_precision.py --task GRAPH-STATE-001 --strict
