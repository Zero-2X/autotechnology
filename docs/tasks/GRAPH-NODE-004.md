# GRAPH-NODE-004：实现 Manual Export 和 FakeOfficialAdapter 节点
...
实现 Manual Export 和 FakeOfficialAdapter 节点。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-HUMAN-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-NODE-004`
...
python scripts/check_task_card_precision.py --task GRAPH-NODE-004 --strict
