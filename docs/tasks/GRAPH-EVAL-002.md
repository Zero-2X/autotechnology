...
建立节点级和整图质量评测。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-EVAL-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-EVAL-002`
...
python scripts/check_task_card_precision.py --task GRAPH-EVAL-002 --strict
