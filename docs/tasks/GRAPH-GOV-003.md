# GRAPH-GOV-003：定义 GraphRun、GraphAttempt、NodeExecution、HumanInterrupt 业务语义
...
定义 GraphRun、GraphAttempt、NodeExecution、HumanInterrupt 业务语义。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-GOV-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-GOV-003`
...
python scripts/check_task_card_precision.py --task GRAPH-GOV-003 --strict
