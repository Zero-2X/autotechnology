# LC-RETRIEVER-001：实现 Retriever Port、权限过滤和证据引用
...
实现 Retriever Port、权限过滤和证据引用。本任务只处理该目标，不跨任务重构。
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `LC-RETRIEVER-001`
...
python scripts/check_task_card_precision.py --task LC-RETRIEVER-001 --strict
