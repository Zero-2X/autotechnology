# GRAPH-DIST-002：实现 OAuth/Vault/Scope/租约检查
...
实现 OAuth/Vault/Scope/租约检查。本任务只处理该目标，不跨任务重构。

## 输入

- 前置任务输出：GRAPH-DIST-001
...
- **Given** 依赖已完成且输入契约有效
- **When** 通过公开 Use Case/GraphRunner 执行 `GRAPH-DIST-002`
...
python scripts/check_task_card_precision.py --task GRAPH-DIST-002 --strict
