...
$webBuild = Join-Path $root '.local\web-console-dist'
...
if (-not (Test-LocalEndpoint "http://127.0.0.1:$WebPort/")) {
    Start-Process -WindowStyle Hidden -FilePath python -ArgumentList @('-m','http.server',"$WebPort",'--bind','127.0.0.1','--directory',$webBuild) -WorkingDirectory $root
}
$webReady = $false
for ($i = 0; $i -lt 20; $i++) {
